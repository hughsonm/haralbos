...
def series_win_chance_home_away(win_chance_home, win_chance_away, wins_remaining, losses_remaining):
    game_at_home = [True, True, False, False, False, True, True]
    if losses_remaining == 0:
        return 0.0
    if wins_remaining == 0:
        return 1.0
    game_index = len(game_at_home) + 1 - (losses_remaining+wins_remaining)
    win_chance = win_chance_home if game_at_home[game_index] else win_chance_away
    series_chance_if_win = series_win_chance_home_away(win_chance_home, win_chance_away, wins_remaining-1, losses_remaining)
    series_chance_if_lose = series_win_chance_home_away(win_chance_home, win_chance_away, wins_remaining, losses_remaining-1)
...
def trophy_chance(game_chance, wins_remaining, losses_remaining):
    return series_win_chance_home_away(game_chance, game_chance, wins_remaining, losses_remaining)
...
def gem_payout(cc, ww, ll):
    ...
    # plus gem-value of packs
    payouts_by_remaining_wins = [1300+3*100, 1000+2*100, 800+100, 150+100, 50+100]
    if ll == 0:
...
def main():
    ...
    while(tolerance < err):
        ...
        midpoint_payout = gem_payout(midpoint, wins_remaining, losses_remaining)
    ...
    print(f'With a win-rate of {break_even_point:.3f} you earn {gem_payout(break_even_point, wins_remaining, losses_remaining)} gems per draft')
    pass
